...
export default async function addTags(lore_id, tags) {
    ...
    const addedTag = arr.reduce(async (accumulator, tag_title) => {
        // Create a tag if it doesn't exist
        const [tag, createdTag] = await Tags.findOrCreate({
            where: {
                tag_title
            },
            defaults: {
        ...
        const [loreTag, createdLoreTag] = await LoreTags.findOrCreate({
            where: {
                tag_id,
                lore_id
            }, 
